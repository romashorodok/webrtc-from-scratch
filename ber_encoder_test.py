...
from webrtc.dtls.pyasn1 import char
from webrtc.dtls.pyasn1 import ber_encoder
...
class LargeTagEncoderTestCase(BaseTestCase):
    ...
    def testEncoder(self):
        assert ber_encoder.encode(self.o) == bytes(
...
class IntegerEncoderTestCase(BaseTestCase):
    def testPosInt(self):
        assert ber_encoder.encode(univ.Integer(12)) == bytes((2, 1, 12))

    def testNegInt(self):
        assert ber_encoder.encode(univ.Integer(-12)) == bytes((2, 1, 244))

    def testZero(self):
        assert ber_encoder.encode(univ.Integer(0)) == bytes((2, 1, 0))

    def testCompactZero(self):
        ber_encoder.IntegerEncoder.supportCompactZero = True
        substrate = ber_encoder.encode(univ.Integer(0))
        ber_encoder.IntegerEncoder.supportCompactZero = False
        assert substrate == bytes((2, 0))

    def testMinusOne(self):
        assert ber_encoder.encode(univ.Integer(-1)) == bytes((2, 1, 255))

    def testPosLong(self):
        assert ber_encoder.encode(univ.Integer(0xFFFFFFFFFFFFFFFF)) == bytes(
...
class IntegerEncoderWithSchemaTestCase(BaseTestCase):
    def testPosInt(self):
        assert ber_encoder.encode(12, asn1Spec=univ.Integer()) == bytes((2, 1, 12))

    def testNegInt(self):
        assert ber_encoder.encode(-12, asn1Spec=univ.Integer()) == bytes((2, 1, 244))

    def testZero(self):
        assert ber_encoder.encode(0, asn1Spec=univ.Integer()) == bytes((2, 1, 0))

    def testPosLong(self):
        assert ber_encoder.encode(0xFFFFFFFFFFFFFFFF, asn1Spec=univ.Integer()) == bytes(
            (2, 9, 0, 255, 255, 255, 255, 255, 255, 255, 255)
        )
...
class BooleanEncoderTestCase(BaseTestCase):
    def testTrue(self):
        assert ber_encoder.encode(univ.Boolean(1)) == bytes((1, 1, 1))

    def testFalse(self):
        assert ber_encoder.encode(univ.Boolean(0)) == bytes((1, 1, 0))


class BooleanEncoderWithSchemaTestCase(BaseTestCase):
    def testTrue(self):
        assert ber_encoder.encode(True, asn1Spec=univ.Boolean()) == bytes((1, 1, 1))

    def testFalse(self):
        assert ber_encoder.encode(False, asn1Spec=univ.Boolean()) == bytes((1, 1, 0))
...
class BitStringEncoderTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        assert ber_encoder.encode(self.b) == bytes((3, 3, 1, 169, 138))

    def testIndefMode(self):
        assert ber_encoder.encode(self.b, defMode=False) == bytes((3, 3, 1, 169, 138))

    def testDefModeChunked(self):
        assert ber_encoder.encode(self.b, maxChunkSize=1) == bytes(
            (35, 8, 3, 2, 0, 169, 3, 2, 1, 138)
        )

    def testIndefModeChunked(self):
        assert ber_encoder.encode(self.b, defMode=False, maxChunkSize=1) == bytes(
            (35, 128, 3, 2, 0, 169, 3, 2, 1, 138, 0, 0)
        )

    def testEmptyValue(self):
        assert ber_encoder.encode(univ.BitString([])) == bytes((3, 1, 0))
...
class BitStringEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        assert ber_encoder.encode(self.b, asn1Spec=self.s) == bytes((3, 3, 1, 169, 138))

    def testIndefMode(self):
        assert ber_encoder.encode(self.b, asn1Spec=self.s, defMode=False) == bytes(
            (3, 3, 1, 169, 138)
        )

    def testDefModeChunked(self):
        assert ber_encoder.encode(self.b, asn1Spec=self.s, maxChunkSize=1) == bytes(
            (35, 8, 3, 2, 0, 169, 3, 2, 1, 138)
        )

    def testIndefModeChunked(self):
        assert ber_encoder.encode(
            self.b, asn1Spec=self.s, defMode=False, maxChunkSize=1
        ) == bytes((35, 128, 3, 2, 0, 169, 3, 2, 1, 138, 0, 0))

    def testEmptyValue(self):
        assert ber_encoder.encode([], asn1Spec=self.s) == bytes((3, 1, 0))
...
class OctetStringEncoderTestCase(BaseTestCase):
    ...
    def testIndefMode(self):
        assert ber_encoder.encode(self.o, defMode=False) == bytes(
            (
                4,
                15,
                81,
    ...
    def testDefModeChunked(self):
        assert ber_encoder.encode(self.o, maxChunkSize=4) == bytes(
            (
                36,
                23,
                4,
    ...
    def testIndefModeChunked(self):
        assert ber_encoder.encode(self.o, defMode=False, maxChunkSize=4) == bytes(
            (
                36,
                128,
                4,
...
class OctetStringEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        assert ber_encoder.encode(self.o, asn1Spec=self.s) == bytes(
            (
                4,
                15,
                81,
    ...
    def testIndefMode(self):
        assert ber_encoder.encode(self.o, asn1Spec=self.s, defMode=False) == bytes(
            (
                4,
                15,
                81,
    ...
    def testDefModeChunked(self):
        assert ber_encoder.encode(self.o, asn1Spec=self.s, maxChunkSize=4) == bytes(
            (
                36,
                23,
                4,
...
class ExpTaggedOctetStringEncoderTestCase(BaseTestCase):
    ...
    def testDefModeChunked(self):
        assert ber_encoder.encode(self.o, defMode=True, maxChunkSize=4) == bytes(
            (
                101,
                25,
                36,
...
class NullEncoderTestCase(BaseTestCase):
    def testNull(self):
        assert ber_encoder.encode(univ.Null("")) == bytes((5, 0))


class NullEncoderWithSchemaTestCase(BaseTestCase):
    def testNull(self):
        assert ber_encoder.encode(None, univ.Null()) == bytes((5, 0))
...
class ObjectIdentifierEncoderTestCase(BaseTestCase):
    ...
    def testEdge1(self):
        assert ber_encoder.encode(univ.ObjectIdentifier((0, 39))) == bytes((6, 1, 39))

    def testEdge2(self):
        assert ber_encoder.encode(univ.ObjectIdentifier((1, 39))) == bytes((6, 1, 79))

    def testEdge3(self):
        # 01111111
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 40))) == bytes((6, 1, 120))

    def testEdge4(self):
        # 10010000|10000000|10000000|10000000|01001111
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 0xFFFFFFFF))) == bytes(
            (6, 5, 0x90, 0x80, 0x80, 0x80, 0x4F)
        )

    def testEdge5(self):
        # 01111111
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 47))) == bytes((6, 1, 0x7F))

    def testEdge6(self):
        # 10000001|00000000
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 48))) == bytes(
            (6, 2, 0x81, 0x00)
        )

    def testEdge7(self):
        # 10000001|00110100|00000003
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 100, 3))) == bytes(
            (6, 3, 0x81, 0x34, 0x03)
        )

    def testEdge8(self):
        # 10000101|00000000
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 560))) == bytes(
            (6, 2, 133, 0)
        )

    def testEdge9(self):
        # 10001000|10000100|10000111|0000010
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 16843570))) == bytes(
            (6, 4, 0x88, 0x84, 0x87, 0x02)
        )

    def testEdgeA(self):
        assert ber_encoder.encode(univ.ObjectIdentifier((2, 5))) == bytes((6, 1, 85))

    def testImpossible1(self):
        try:
            ber_encoder.encode(univ.ObjectIdentifier((3, 1, 2)))
        except PyAsn1Error:
            pass
        else:
    ...
    def testImpossible2(self):
        try:
            ber_encoder.encode(univ.ObjectIdentifier((0,)))
        except PyAsn1Error:
            pass
        else:
    ...
    def testImpossible3(self):
        try:
            ber_encoder.encode(univ.ObjectIdentifier((0, 40)))
        except PyAsn1Error:
            pass
        else:
    ...
    def testImpossible4(self):
        try:
            ber_encoder.encode(univ.ObjectIdentifier((1, 40)))
        except PyAsn1Error:
            pass
        else:
            assert 0, "second arc overflow tolerated"
...
class RelativeOIDEncoderTestCase(BaseTestCase):
    def testOne(self):
        assert ber_encoder.encode(univ.RelativeOID((39,))) == bytes((13, 1, 39))

    def testTwo(self):
        assert ber_encoder.encode(univ.RelativeOID((5, 6))) == bytes((13, 2, 5, 6))

    def testThree(self):
        assert ber_encoder.encode(univ.RelativeOID((5, 6, 7))) == bytes(
            (13, 3, 5, 6, 7)
        )
...
class RelativeOIDWithSchemaEncoderTestCase(BaseTestCase):
    def testOne(self):
        assert ber_encoder.encode((5, 6, 7), asn1Spec=univ.RelativeOID()) == bytes(
            (13, 3, 5, 6, 7)
        )
...
class RealEncoderTestCase(BaseTestCase):
    def testChar(self):
        assert ber_encoder.encode(univ.Real((123, 10, 11))) == bytes(
            (9, 7, 3, 49, 50, 51, 69, 49, 49)
        )

    def testBin1(self):
        assert ber_encoder.encode(  # default binEncBase = 2
    ...
    def testBin3(self):
        ...
        binEncBase, ber_encoder.TYPE_MAP[univ.Real.typeId].binEncBase = (
            ber_encoder.TYPE_MAP[univ.Real.typeId].binEncBase,
            16,
        )
        assert ber_encoder.encode(
            univ.Real((0.00390625, 2, 0))  # check encbase = 16
        ) == bytes((9, 3, 160, 254, 1))
        ber_encoder.TYPE_MAP[univ.Real.typeId].binEncBase = binEncBase
    ...
    def testPlusInf(self):
        assert ber_encoder.encode(univ.Real("inf")) == bytes((9, 1, 64))

    def testMinusInf(self):
        assert ber_encoder.encode(univ.Real("-inf")) == bytes((9, 1, 65))

    def testZero(self):
        assert ber_encoder.encode(univ.Real(0)) == bytes((9, 0))


class RealEncoderWithSchemaTestCase(BaseTestCase):
    def testChar(self):
        assert ber_encoder.encode((123, 10, 11), asn1Spec=univ.Real()) == bytes(
            (9, 7, 3, 49, 50, 51, 69, 49, 49)
        )
...
class UniversalStringEncoderTestCase(BaseTestCase):
    def testEncoding(self):
        assert ber_encoder.encode(char.UniversalString("abc")) == bytes(
...
class UniversalStringEncoderWithSchemaTestCase(BaseTestCase):
    def testEncoding(self):
        assert ber_encoder.encode("abc", asn1Spec=char.UniversalString()) == bytes(
            (28, 12, 0, 0, 0, 97, 0, 0, 0, 98, 0, 0, 0, 99)
        ), "Incorrect encoding"
...
class BMPStringEncoderTestCase(BaseTestCase):
    def testEncoding(self):
        assert ber_encoder.encode(char.BMPString("abc")) == bytes(
            (30, 6, 0, 97, 0, 98, 0, 99)
        ), "Incorrect encoding"
...
class BMPStringEncoderWithSchemaTestCase(BaseTestCase):
    def testEncoding(self):
        assert ber_encoder.encode("abc", asn1Spec=char.BMPString()) == bytes(
            (30, 6, 0, 97, 0, 98, 0, 99)
        ), "Incorrect encoding"
...
class UTF8StringEncoderTestCase(BaseTestCase):
    def testEncoding(self):
        assert ber_encoder.encode(char.UTF8String("abc")) == bytes(
            (12, 3, 97, 98, 99)
        ), "Incorrect encoding"
...
class UTF8StringEncoderWithSchemaTestCase(BaseTestCase):
    def testEncoding(self):
        assert ber_encoder.encode("abc", asn1Spec=char.UTF8String()) == bytes(
            (12, 3, 97, 98, 99)
        ), "Incorrect encoding"
...
class SequenceOfEncoderTestCase(BaseTestCase):
    def testEmpty(self):
        s = univ.SequenceOf()
        s.clear()
        assert ber_encoder.encode(s) == bytes((48, 0))

    def testDefMode(self):
        s = univ.SequenceOf()
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert ber_encoder.encode(s) == bytes(
    ...
    def testIndefMode(self):
        s = univ.SequenceOf()
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert ber_encoder.encode(s, defMode=False) == bytes(
    ...
    def testDefModeChunked(self):
        s = univ.SequenceOf()
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert ber_encoder.encode(s, defMode=True, maxChunkSize=4) == bytes(
            (
                48,
                19,
                36,
    ...
    def testIndefModeChunked(self):
        s = univ.SequenceOf()
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert ber_encoder.encode(s, defMode=False, maxChunkSize=4) == bytes(
            (
                48,
                128,
                36,
...
class SequenceOfEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testEmpty(self):
        assert ber_encoder.encode([], asn1Spec=self.s) == bytes((48, 0))

    def testDefMode(self):
        assert ber_encoder.encode(self.v, asn1Spec=self.s) == bytes(
            (48, 13, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110)
        )

    def testIndefMode(self):
        assert ber_encoder.encode(self.v, asn1Spec=self.s, defMode=False) == bytes(
            (48, 128, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 0, 0)
        )
...
class SequenceOfEncoderWithComponentsSchemaTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        self.__init()
        assert ber_encoder.encode(self.s) == bytes(
            (48, 13, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110)
        )

    def testIndefMode(self):
        self.__init()
        assert ber_encoder.encode(self.s, defMode=False) == bytes(
            (48, 128, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 0, 0)
        )

    def testDefModeChunked(self):
        self.__init()
        assert ber_encoder.encode(self.s, defMode=True, maxChunkSize=4) == bytes(
            (
                48,
                19,
                36,
    ...
    def testIndefModeChunked(self):
        self.__init()
        assert ber_encoder.encode(self.s, defMode=False, maxChunkSize=4) == bytes(
            (
                48,
                128,
                36,
...
class SetOfEncoderTestCase(BaseTestCase):
    def testEmpty(self):
        s = univ.SetOf()
        s.clear()
        assert ber_encoder.encode(s) == bytes((49, 0))
...
class SetOfEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testEmpty(self):
        s = univ.SetOf()
        assert ber_encoder.encode([], asn1Spec=self.s) == bytes((49, 0))
...
class SequenceEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testEmpty(self):
        try:
            assert ber_encoder.encode({}, asn1Spec=self.s)
...
class SequenceEncoderWithUntaggedOpenTypesTestCase(BaseTestCase):
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert ber_encoder.encode(self.s, asn1Spec=self.s) == bytes(
            (48, 5, 2, 1, 1, 49, 50)
        )
    ...
    def testEncodeOpenTypeUnknownId(self):
        ...
        try:
            ber_encoder.encode(self.s, asn1Spec=self.s)
...
class SequenceEncoderWithComponentsSchemaTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        self.__init()
        assert ber_encoder.encode(self.s) == bytes((48, 2, 5, 0))

    def testIndefMode(self):
        self.__init()
        assert ber_encoder.encode(self.s, defMode=False) == bytes((48, 128, 5, 0, 0, 0))
    ...
    def testWithDefaultedDefMode(self):
        ...
        assert ber_encoder.encode(self.s) == bytes((48, 5, 5, 0, 2, 1, 1))
...
class ExpTaggedSequenceEncoderTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        assert ber_encoder.encode(self.s) == bytes((101, 5, 48, 3, 2, 1, 12))
...
class ExpTaggedSequenceComponentEncoderTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        assert ber_encoder.encode(self.s) == bytes((48, 5, 160, 3, 1, 1, 1))
...
class SetEncoderWithComponentsSchemaTestCase(BaseTestCase):
    ...
    def testDefMode(self):
        self.__init()
        assert ber_encoder.encode(self.s) == bytes((49, 2, 5, 0))

    def testIndefMode(self):
        self.__init()
        assert ber_encoder.encode(self.s, defMode=False) == bytes((49, 128, 5, 0, 0, 0))
    ...
    def testWithDefaultedDefMode(self):
        self.__initWithDefaulted()
        assert ber_encoder.encode(self.s) == bytes((49, 5, 5, 0, 2, 1, 1))
...
class ChoiceEncoderTestCase(BaseTestCase):
    ...
    def testDefModeOptionOne(self):
        ...
        assert ber_encoder.encode(s) == bytes((5, 0))
...
class ChoiceEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testFilled(self):
        assert ber_encoder.encode(self.v, asn1Spec=self.s) == bytes((5, 0))
...
class ChoiceEncoderWithComponentsSchemaTestCase(BaseTestCase):
    ...
    def testFilled(self):
        self.s.setComponentByPosition(0, univ.Null(""))
        assert ber_encoder.encode(self.s) == bytes((5, 0))
    ...
    def testTagged(self):
        ...
        s.setComponentByPosition(0, univ.Null(""))
        assert ber_encoder.encode(s) == bytes((164, 2, 5, 0))

    def testUndefLength(self):
        self.s.setComponentByPosition(2, univ.OctetString("abcdefgh"))
        assert ber_encoder.encode(self.s, defMode=False, maxChunkSize=3) == bytes(
    ...
    def testTaggedUndefLength(self):
        ...
        assert ber_encoder.encode(s, defMode=False, maxChunkSize=3) == bytes(
            (
                164,
                128,
                36,
...
class AnyEncoderTestCase(BaseTestCase):
    def setUp(self):
        BaseTestCase.setUp(self)
        self.s = univ.Any(ber_encoder.encode(univ.OctetString("fox")))

    def testUntagged(self):
        assert ber_encoder.encode(self.s) == bytes((4, 3, 102, 111, 120))

    def testTaggedEx(self):
        s = self.s.subtype(
            explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 4)
        )
        assert ber_encoder.encode(s) == bytes((164, 5, 4, 3, 102, 111, 120))

    def testTaggedIm(self):
        s = self.s.subtype(
            implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 4)
        )
        assert ber_encoder.encode(s) == bytes((132, 5, 4, 3, 102, 111, 120))
...
class AnyEncoderWithSchemaTestCase(BaseTestCase):
    def setUp(self):
        BaseTestCase.setUp(self)
        self.s = univ.Any()
        self.v = ber_encoder.encode(univ.OctetString("fox"))
    ...
    def testTaggedEx(self):
        ...
        assert ber_encoder.encode(self.v, asn1Spec=s) == bytes(
