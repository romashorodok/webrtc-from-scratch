...
class OctetStringEncoderTestCase(BaseTestCase):
    ...
    def testDefModeLong(self):
        assert der_encoder.encode(univ.OctetString("Q" * 10000)) == bytes(
...
class BitStringEncoderTestCase(BaseTestCase):
    def testDefModeShort(self):
        assert der_encoder.encode(univ.BitString((1,))) == bytes((3, 2, 7, 128))

    def testDefModeLong(self):
        assert der_encoder.encode(univ.BitString((1,) * 80000)) == bytes(
...
class SetOfEncoderTestCase(BaseTestCase):
    ...
    def testDefMode1(self):
        ...
        assert der_encoder.encode(self.s) == bytes((49, 7, 4, 1, 97, 4, 2, 97, 98))
    ...
    def testDefMode3(self):
        ...
        assert der_encoder.encode(self.s) == bytes((49, 6, 4, 1, 97, 4, 1, 98))
...
class SetWithAlternatingChoiceEncoderTestCase(BaseTestCase):
    ...
    def testComponentsOrdering1(self):
        ...
        assert der_encoder.encode(self.s) == bytes((49, 6, 2, 1, 5, 4, 1, 65))

    def testComponentsOrdering2(self):
        self.s.setComponentByName("status")
        self.s.getComponentByName("status").setComponentByPosition(1, True)
        assert der_encoder.encode(self.s) == bytes((49, 6, 1, 1, 255, 2, 1, 5))
...
class SetWithTaggedChoiceEncoderTestCase(BaseTestCase):
    def testWithUntaggedChoice(self):
        ...
        assert der_encoder.encode(s) == bytes((49, 6, 1, 1, 255, 4, 1, 65))
    ...
    def testWithTaggedChoice(self):
        ...
        assert der_encoder.encode(s) == bytes((49, 8, 4, 1, 65, 167, 3, 1, 1, 255))
...
class SequenceEncoderWithUntaggedOpenTypesTestCase(BaseTestCase):
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert der_encoder.encode(self.s, asn1Spec=self.s) == bytes(
            (48, 5, 2, 1, 1, 49, 50)
        )
    ...
    def testEncodeOpenTypeUnknownId(self):
        ...
        try:
            der_encoder.encode(self.s, asn1Spec=self.s)
...
class EmptyInnerFieldOfSequenceEncoderTestCase(BaseTestCase):
    def testInitializedOptionalNullIsEncoded(self):
        ...
        self.s.clear()
        self.s[0] = ""
        assert der_encoder.encode(self.s) == bytes((48, 2, 5, 0))
    ...
    def testUninitializedOptionalNullIsNotEncoded(self):
        ...
        self.s.clear()
        assert der_encoder.encode(self.s) == bytes((48, 0))
    ...
    def testInitializedOptionalOctetStringIsEncoded(self):
        ...
        self.s.clear()
        self.s[0] = ""
        assert der_encoder.encode(self.s) == bytes((48, 2, 4, 0))
...
class ClassConstructorTestCase(BaseTestCase):
    def testKeywords(self):
        ...
        sie = der_encoder.Encoder()._singleItemEncoder
        self.assertIs(sie._tagMap, der_encoder.TAG_MAP)
        self.assertIs(sie._typeMap, der_encoder.TYPE_MAP)

        sie = der_encoder.Encoder(tagMap=tagmap, typeMap=typemap)._singleItemEncoder
        self.assertIs(sie._tagMap, tagmap)
        self.assertIs(sie._typeMap, typemap)

        sie = der_encoder.Encoder(tagmap, typemap)._singleItemEncoder
        self.assertIs(sie._tagMap, tagmap)
        self.assertIs(sie._typeMap, typemap)
