...
from webrtc.dtls.pyasn1 import cer_decoder
from webrtc.dtls.pyasn1.error import PyAsn1Error


class BooleanDecoderTestCase(BaseTestCase):
    def testTrue(self):
        assert cer_decoder.decode(bytes((1, 1, 255))) == (1, b"")

    def testFalse(self):
        assert cer_decoder.decode(bytes((1, 1, 0))) == (0, b"")

    def testEmpty(self):
        try:
            cer_decoder.decode(bytes((1, 0)))
        except PyAsn1Error:
            pass

    def testOverflow(self):
        try:
            cer_decoder.decode(bytes((1, 2, 0, 0)))
        except PyAsn1Error:
            pass
...
class BitStringDecoderTestCase(BaseTestCase):
    def testShortMode(self):
        assert cer_decoder.decode(bytes((3, 3, 6, 170, 128))) == (((1, 0) * 5), b"")

    def testLongMode(self):
        assert cer_decoder.decode(bytes((3, 127, 6) + (170,) * 125 + (128,))) == (
            ((1, 0) * 501),
            b"",
        )
...
class SequenceDecoderWithUntaggedSetOfOpenTypesTestCase(BaseTestCase):
    ...
    def testDontDecodeOpenTypesChoiceOne(self):
        s, r = cer_decoder.decode(
            bytes((48, 128, 2, 1, 1, 49, 128, 2, 1, 12, 0, 0, 0, 0)), asn1Spec=self.s
        )
        assert not r
        assert s[0] == 1
