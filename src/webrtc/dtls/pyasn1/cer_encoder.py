...
class BooleanEncoder(ber_encoder.IntegerEncoder):
...
class RealEncoder(ber_encoder.RealEncoder):
...
class TimeEncoderMixIn(object):
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        return ber_encoder.OctetStringEncoder.encodeValue(
            self, value, asn1Spec, encodeFun, **options
        )


class GeneralizedTimeEncoder(TimeEncoderMixIn, ber_encoder.OctetStringEncoder):
    MIN_LENGTH = 12
    MAX_LENGTH = 20


class UTCTimeEncoder(TimeEncoderMixIn, ber_encoder.OctetStringEncoder):
    MIN_LENGTH = 10
    MAX_LENGTH = 14


class SetOfEncoder(ber_encoder.SequenceOfEncoder):
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
...
class SequenceOfEncoder(ber_encoder.SequenceOfEncoder):
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
...
class SetEncoder(ber_encoder.SequenceEncoder):
    @staticmethod
...
class SequenceEncoder(ber_encoder.SequenceEncoder):
    omitEmptyOptionals = True


TAG_MAP = ber_encoder.TAG_MAP.copy()
...
TYPE_MAP = ber_encoder.TYPE_MAP.copy()
...
class SingleItemEncoder(ber_encoder.SingleItemEncoder):
...
class Encoder(ber_encoder.Encoder):
