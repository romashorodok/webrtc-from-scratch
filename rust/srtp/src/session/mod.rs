...
pub struct Session {
    ...
    pub(crate) udp_tx: Arc<mpsc::Sender<Vec<u8>>>,
    is_rtp: bool,
}
...
impl Session {
    ...
    async fn incoming(
        ...
        udp_rx: &mut MutexGuard<'_, mpsc::Receiver<Vec<u8>>>,
    ...
    pub async fn write(&self, buf: &Bytes, is_rtp: bool) -> Result<()> {
        ...
        self.udp_tx.send(encrypted.to_vec()).await?;
        Ok(())
    }
