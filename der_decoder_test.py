...
class BitStringDecoderTestCase(BaseTestCase):
    def testShortMode(self):
        assert der_decoder.decode(bytes((3, 127, 6) + (170,) * 125 + (128,))) == (
            ((1, 0) * 501),
            b"",
        )
    ...
    def testIndefMode(self):
        try:
            der_decoder.decode(bytes((35, 128, 3, 2, 0, 169, 3, 2, 1, 138, 0, 0)))
        except PyAsn1Error:
            pass
        else:
    ...
    def testDefModeChunked(self):
        try:
            assert der_decoder.decode(bytes((35, 8, 3, 2, 0, 169, 3, 2, 1, 138)))
        except PyAsn1Error:
            pass
        else:
...
class OctetStringDecoderTestCase(BaseTestCase):
    def testShortMode(self):
        assert der_decoder.decode("\004\017Quick brown fox".encode()) == (
...
class SequenceDecoderWithUntaggedOpenTypesTestCase(BaseTestCase):
    ...
    def testDontDecodeOpenTypesChoiceOne(self):
        s, r = der_decoder.decode(bytes((48, 6, 2, 1, 1, 2, 1, 12)), asn1Spec=self.s)
        assert not r
        assert s[0] == 1
